...
function generateTabletXMLFile(session, config) {
    ...
    if (config.invokeTargets) {
        var invokeTargetsArr = Object.getOwnPropertyNames(config.invokeTargets);

        for(element in config.invokeTargets) {
            console.log("element: " + element);
        }

        xmlObject.invokeTarget = [];
        
        console.log("before");

        invokeTargetsArr.forEach(function (invokeTarget) {
            var xmlInvokeTarget = {
            ...
            if (invokeTarget.permissions) {
                xmlInvokeTarget["require-source-capabilities"] =
                    invokeTarget.permissions.reduce(function (prev, current) {
                        return prev + "," + current;
                    });
            }
                ...
                invokeTarget.filters.forEach(function (filter) {
                    ...
                    if (filter.extensions) {
                        ...
                        filter.extensions.forEach(function (extension) {
                            xmlFilter.property.push({
                                _attr: { var : "exts", value : extension }
                            });
                        });
                    }
            ...
            xmlObject.invokeTarget.push(xmlInvokeTarget);
