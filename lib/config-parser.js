...
function processInvokeTargetsData(data, widgetConfig) {
    ...
    if (data["rim:invokeTargets"] && data["rim:invokeTargets"]["invokeTarget"]) {
        var invokeTargets = data["rim:invokeTargets"]["invokeTarget"];
        
        console.log("type: " + data["rim:invokeTargets"]["invokeTarget"]["@"].invokeType);
        console.log("id: " + data["rim:invokeTargets"]["invokeTarget"]["@"].invokeId);
        if (invokeTargets instanceof Array) {
            widgetConfig.invokeTargets = invokeTargets;
            console.log("true");
        } else {
            widgetConfig.invokeTargets = [invokeTargets];
            console.log("false");
        ...
        widgetConfig.invokeTargets.forEach(function (invokeTarget) {
            ...
            if (invokeTarget["permissions"] && invokeTarget["permissions"] instanceof Array) {
        ...
        widgetConfig.invokeTargets = [];
    }
}

function validateInvokeTargets(widgetConfig) {
    if (widgetConfig["invokeTargets"]) {
    console.log("here");
        check(widgetConfig["invokeTargets"]["invokeTarget"], localize("EXCEPTION_INVOKE_TARGETS_EMPTY"))
            .notNull()
            .notEmpty();
        
        var invokeTargets = widgetConfig["invokeTargets"];
        console.log("before");
        invokeTargets.forEach(function (invokeTarget, index, invokeTargets) {
            var attribs = invokeTarget["@"];
            check(attribs, localize("EXCEPTION_INVALID_INVOKE_TARGET_NO_ATTRIBUTES"))
                .notNull()
                .notEmpty()
                .isArray();
            check(attribs["invokeType"], localize("EXCEPTION_INVOKE_TYPE_ATTRIB_INVALID"))
                .isIn(["application", "viewer"]);
            check(attribs["invokeId"], localize("EXCEPTION_INVOKE_ID_ATTRIB_INVALID"))
                .notEmpty();
                
            //Validate the permissions element further when functionality defined
            if (invokeTarget["permissions"]) {
                if (invokeTarget["permissions"]["permission"]) {
...
function validateConfig(widgetConfig) {
    ...
    //validate invokeTargets
    //validateInvokeTargets(widgetConfig);
}
